...
# 1. Calcul du nombre de sinistres
y_train["NB_SINISTRES"] = y_train["FREQ"] * X_train["ANNEE_ASSURANCE"]

# 2. Encodage en 2 classes : 0 (pas de sinistre) et 1 (au moins un sinistre)
...
# Cramér's V pour les catégorielles nominales
def cramers_v(x, y):
...
for col in ordinal_columns:
    try:
        vord = cramers_v(X_train[col], y_train["SINISTRE"])
        results_cramer_ord.append((col, vord))
    except:
        continue
...
# Conversions en DataFrames triés
df_cat = pd.DataFrame(results_cramer_cat, columns=["Variable_Cat", "Cramers_V"]).sort_values(by="Cramers_V", ascending=False)
df_ord = pd.DataFrame(results_cramer_ord, columns=["Variable_Ord", "Cramers_V"]).sort_values(by="Cramers_V", ascending=False)
df_con = pd.DataFrame(results_kruskal, columns=["Variable_Con", "H_stat", "p_value"]).sort_values(by="p_value")

df_cat = df_cat[df_cat["Cramers_V"] > 0.01]
df_cat
df_ord = df_ord[df_ord["Cramers_V"] > 0.01]
df_ord
df_con = df_con[(df_con["p_value"] < 0.05) & (df_con["H_stat"] > 300)]
df_con

# 2. Fusion des noms de colonnes sélectionnées
selected_vars = (
    df_cat["Variable"].tolist() +
    df_ord["Variable"].tolist() +
    df_con["Variable"].tolist()
)
# Colonnes toujours à inclure
